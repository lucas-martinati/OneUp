...
> **Bootstrap admin (ordre impératif) :**
> 1. `npm run secrets:set` (configure `ADMIN_UID`)
> 2. `npm run deploy:functions`
> 3. Grant the claim — deploying alone does NOT grant it. Either call `grantAdminRole` (httpsCallable, only the account matching `ADMIN_UID` is allowed) or run the one-off Admin SDK script (service account key from Firebase console → Project settings → Service accounts → Generate new private key, keep it out of the repo):
>    ```bash
>    FIREBASE_SERVICE_ACCOUNT=/path/to/serviceAccountKey.json \
>    ADMIN_UID=<your-uid> \
>    node firebase/functions/grant-admin-role.mjs
>    ```
> 4. **refresh the ID token** with `getIdToken(true)` (or sign out/in) — custom claims only appear on fresh tokens; otherwise ~1h wait
> 5. `npm run deploy:database` to publish the new rules. Do NOT deploy the rules before the claim is granted — the hardcoded UID is removed from the rules, so admin writes would be denied.
